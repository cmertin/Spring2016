...
int main(int argc, char *argv[])
{
  ...
  train.clearance = train.startY - baseGround;
  ...
  */
  ...
  filename.append("_");
  filename.append(to_string(train.clearance));
  ...
  outfile << "# Clearance: " << train.clearance << endl;
  outfile << "# =================" << endl;
...
void BoundaryConditions(Flow<T> &flow, Dimensions<T> &dim, Train<T> &train, int threads)
{
  ...
  */
  ...
  // Boundary Conditions for the ground
  int baseGround = train.startY - train.clearance;
  #pragma omp parallel num_threads(threads)
  for(int i = 0; i < dim.Nx+2; ++i)
    {
      for(int j = 3; j < baseGround; ++j)
	{
	  if(j == baseGround - 1)
	    {
	      flow.P[i][j] = 0;
