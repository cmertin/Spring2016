...
def ParseFile(filename):
    ...
    return startX,startY,separation,clearance,dx,dy,height,width,Nx,np.array(x),np.array(y),np.array(u),np.array(v),np.array(P),np.array(omega)
...
def CalcLift(P, startX, startY, dx, dy, height, cv_width):
    pbottom = 0
    ptop = 0
    pleft = 0
    pright = 0
    minY = int(startY - cv_width)
    maxY = int(startY + height + 2 * cv_width)
    minX = int(startX - cv_width)
    maxX = int(startX + height + 2 * cv_width)
...
U = 10
Fd_sep = 33.5486
base_filename = "train_" + str(U) + "_15_15_11_"
velocities = range(5,101,5)
velocities_true = range(0,101)
sep_list = range(1,25,2)
fd_true = []
error = 0.033
fd = []
fd_err = []
...
for sep in sep_list:
    ...
    print("Clearance " + str(sep) + ": ", end="")
    startX,startY,separation,clearance,dx,dy,height,width,Nx,x,y,u,v,P,omega = ParseFile(filename)
    ...
    alpha = CalcAlpha(u, v, dx, dy, startX, startY, height, width_do)
    P_area = CalcPressure(P, startX, startY, dx, dy, height, width_do)
    Lift = CalcLift(P, startX, startY, dx, dy, height, width_do)
    fd.append(P_area + alpha)
    fd_err.append((P_area + alpha)*error)
    fd_sub.append(P_area + alpha - exact)
    fd_sub_err.append((P_area + alpha - exact)*error)
    fd_ratio_sep.append((P_area + alpha)/Fd_sep)
    fd_ratio.append((P_area + alpha - Lift)/exact)
    fd_ratio_err.append((P_area + alpha)/exact * error)
    error_d = "%.5f" % (error * (P_area + alpha))
    error_dd = "%.5f" % (error * (P_area + alpha - exact))
    drag = "%.4f" % (P_area + alpha - Lift)
    d_drag = "%.4f" % (P_area + alpha - exact)
...
print("=====\n\n")
print("\\begin{tikzpicture}")
print("\\begin{axis}[xlabel={$\\frac{Separation}{Width}$}, ylabel={$\\frac{F_{d}^{(clearance)}}{F_{d}}$}, title={Ratio of Train Car to Single Block}, y tick label style={/pgf/number format/.cd,fixed,fixed zerofill,precision=3,/tikz/.cd}]")#, xtick={",end="")
#for i in xrange(0, len(sep_ratio)):
#    x = "%.4f" % sep_ratio[i]
#    print(x + ",",end="")
#print("}, xticklabels={",end="")
...
print("=========")
print("\\begin{tikzpicture}")
print("\\begin{axis}[xlabel={$\\frac{Separation}{Width}$}, ylabel={$\\frac{F_{d}^{(clearance)}}{F_{d}^{(sep)}}$}, title={Ratio of Train Car to Single Block}, y tick label style={/pgf/number format/.cd,fixed,fixed zerofill,precision=3,/tikz/.cd}]")
print("\\addplot[mark=*, thick, color=blue] coordinates{")
for i in xrange(0, len(fd_ratio_sep)):
    x = "%.4f" % sep_ratio[i]
    y = "%.4f" % fd_ratio_sep[i]
    print("(" + x + "," + y + ")")
print("};")
